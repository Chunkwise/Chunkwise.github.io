...
However, we initially overscaled our jobs – because our team was in OpenAI’s Usage Tier 1, `text-embedding-3-small`’s 1,000,000 tokens-per-minute rate limit was often the bottleneck. We reduced the maximum number of jobs from 32 to 4, but still frequently hit rate limits. We decided against further reducing the number of jobs because organizations in a higher tier would not face the same bottleneck.
